		...
		if (scene == '' || valid == false) {
			...
			$('.hasScale.hasChildren').each(function() {
				...
				children.each(function() {
					staticAttrCh = $(this).attr('static'),
					scaleAttrCh = $(this).attr('scale');

					if (typeof staticAttrCh !== 'undefined' && staticAttrCh !== false && !scaleAttrCh) {
						$(this).closest('.hasScale').addClass('staticActor');
					};
				});
			...
			if (staticActorsFound == 0) {
				...
				$('.staticActor').each(function() {
					...
					if (name) { outputText += "<tr><td>Name</td><td>"+name+"</td></tr>" };
					...
					if (shapeid) {
						outputText += "<div style='text-align:right;'>To find this section in your map i3d, search for <code>shapeid=\""+shapeid+"\"</code></div>";
					} else if (nodeid) {
